...
export function buildDeptAggSummaryHtml(deptValue){
  ...
  const deptSummaryCards = stats.deptOrder.length > 1 ? (
    ...
    + stats.deptOrder.map(dept => {
      const deptStats = stats.itemMap;
      const deptItems = Object.values(deptStats).filter(it => it.dept === dept);
      const deptGood = deptItems.filter(it => it.tier === 'good').length;
      const deptBad = deptItems.filter(it => it.tier === 'bad').length;
      const deptImplRate = deptItems.length > 0 ? Math.round((deptGood + deptItems.filter(it => it.tier === 'neutral').length) / deptItems.length * 100) : 0;
      return '<div class="dept-card">'
        ...
        + '<div class="dept-bad"><span class="num">' + deptBad + '</span><span class="label">미흡</span></div>'
        + '</div>';
    }).join('')
    + '</div>'
  ) : '';
  ...
  return '<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>설문 응답 요약 — ' + esc4(deptLabel) + '</title><style>'
    ...
    + '.filter-panel{background:#f9f6f0;border:1px solid #dcd6c8;border-radius:8px;padding:14px;margin-bottom:20px;display:none;}'
    + '.filter-panel.show{display:block;}'
    + '.filter-row{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;align-items:end;}'
    + '.filter-group{display:flex;flex-direction:column;}'
    + '.filter-group label{font-size:10px;color:#5a6472;text-transform:uppercase;margin-bottom:4px;font-weight:700;}'
    + '.filter-group select,.filter-group input{padding:7px;border:1px solid #c7c1b1;border-radius:4px;font-size:12px;font-family:inherit;}'
    + '.filter-btn{padding:7px 12px;background:#132845;color:#f4efe2;border:none;border-radius:4px;cursor:pointer;font-size:12px;font-weight:600;}'
    + '.filter-btn:hover{background:#1a3a5a;}'
    + '.common-weakness{background:#fff8f6;}'
    + '.common-badge{background:#f7e6e2;color:#a23b2e;padding:2px 6px;border-radius:4px;font-size:9px;font-weight:700;}'
    + '.dept-cards-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:10px;margin-bottom:20px;}'
    + '.dept-card{background:#f9f6f0;border:1px solid #dcd6c8;border-radius:6px;padding:12px;text-align:center;}'
    + '.dept-name{font-size:12px;font-weight:700;color:#132845;margin-bottom:8px;word-break:break-all;}'
    + '.dept-stats{display:flex;justify-content:center;gap:12px;align-items:baseline;margin-bottom:8px;}'
    + '.dept-stats .rate{font-size:18px;font-weight:700;color:#2e7d5b;font-family:\'IBM Plex Mono\',monospace;}'
    + '.dept-stats .label{font-size:10px;color:#5a6472;}'
    + '.dept-bad{font-size:11px;color:#a23b2e;}'
    + '.dept-bad .num{font-size:16px;font-weight:700;font-family:\'IBM Plex Mono\',monospace;}'
    ...
    + '.print-btn{position:fixed;top:16px;right:16px;background:#132845;color:#f4efe2;border:none;border-radius:6px;padding:9px 16px;font-size:12.5px;cursor:pointer;z-index:1000;}'
    + '.print-btn.save-btn{right:150px;background:#4a5a72;}'
    + '.print-btn.filter-toggle-btn{right:284px;background:#463b8a;}'
    + '.print-btn.export-btn{right:418px;background:#5a6472;}'
    + '@media print{.print-btn{display:none;}.filter-panel{display:none;}body{background:#fff;}.wrap{box-shadow:none;margin:0;max-width:none;}}'
    + '</style></head><body>'
